...
public class PlanetWars {
    ...
    //Now done only on the first turn
    // Returns the distance between two planets, rounded up to the next highest
    // integer. This is the number of discrete time steps it takes to get
    // between the two planets.
    private int Distance(int sourcePlanet, int destinationPlanet) {
    ...
    //First turn calculate all distances, prevents unnecessary calcs later on.
    public void calcDistances(){
    	int size = this.planets.size();
    	distances = new int[size][size];
    	for(int i=0;i<size;i++){
    		for(int j=0; j<size;j++){
    			distances[i][j] = this.Distance(i, j);
    		}
    	}
    }
    ...
    // planets and fleets, would we!? I wouldn't mind losing player 2's planets/fleets.
